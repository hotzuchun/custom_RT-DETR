...
class YOLOStyleLogger:
    ...
    def save_training_summary(self, model_info: Dict[str, Any], 
        ...
        with open(self.summary_file, 'w', encoding='utf-8') as f:
            f.write(f"RT-DETR Training Summary\n")
            f.write(f"Experiment: {self.experiment_name}\n")
            f.write(f"Total Training Time: {total_training_time:.2f}s ({total_training_time/3600:.2f}h)\n")
            f.write(f"Best Epoch: {best_metrics.get('best_epoch', 'N/A')}\n")
            f.write(f"Best mAP@0.5: {best_metrics.get('best_map50', 0):.4f}\n")
            f.write(f"Best mAP@0.5:0.95: {best_metrics.get('best_map', 0):.4f}\n")
            f.write(f"\nModel Information:\n")
    ...
    def _create_results_plot(self, df: pd.DataFrame):
        ...
        fig.suptitle(f'RT-DETR Training Results - {self.experiment_name}', fontsize=16)
        
        if 'train/loss' in df.columns:
            axes[0, 0].plot(df['epoch'], df['train/loss'], 'b-', label='Total Loss')
        ...
        if 'val/mAP_0.5' in df.columns:
            axes[0, 1].plot(df['epoch'], df['val/mAP_0.5'], 'r-', label='mAP@0.5')
            axes[0, 1].plot(df['epoch'], df['val/mAP_0.5:0.95'], 'g-', label='mAP@0.5:0.95')
        ...
        if 'val/precision' in df.columns and 'val/recall' in df.columns:
            axes[0, 2].plot(df['epoch'], df['val/precision'], 'b-', label='Precision')
            axes[0, 2].plot(df['epoch'], df['val/recall'], 'r-', label='Recall')
        ...
        if 'lr/pg0' in df.columns:
            axes[0, 3].plot(df['epoch'], df['lr/pg0'], 'g-', label='Learning Rate')
        ...
        if 'train/loss_ce' in df.columns:
            axes[1, 0].plot(df['epoch'], df['train/loss_ce'], 'b-', label='CE Loss')
            axes[1, 0].plot(df['epoch'], df['train/loss_bbox'], 'r-', label='BBox Loss')
            axes[1, 0].plot(df['epoch'], df['train/loss_giou'], 'g-', label='GIoU Loss')
        ...
        if 'time/train_epoch' in df.columns:
            axes[1, 1].plot(df['epoch'], df['time/train_epoch'], 'b-', label='Train Time')
            if 'time/val_epoch' in df.columns:
                axes[1, 1].plot(df['epoch'], df['time/val_epoch'], 'r-', label='Val Time')
    ...
    def _create_loss_plot(self, df: pd.DataFrame):
        ...
        ax.plot(df['epoch'], df['train/loss'], 'b-', linewidth=2, label='Total Loss')
        
        if 'train/loss_ce' in df.columns:
            ax.plot(df['epoch'], df['train/loss_ce'], 'r--', alpha=0.7, label='CE Loss')
        if 'train/loss_bbox' in df.columns:
            ax.plot(df['epoch'], df['train/loss_bbox'], 'g--', alpha=0.7, label='BBox Loss')
        if 'train/loss_giou' in df.columns:
            ax.plot(df['epoch'], df['train/loss_giou'], 'y--', alpha=0.7, label='GIoU Loss')
        
        ax.set_title(f'Training Loss - {self.experiment_name}', fontsize=14)
    ...
    def _create_map_plot(self, df: pd.DataFrame):
        if 'val/mAP_0.5' not in df.columns:
            return
            
        fig, ax = plt.subplots(figsize=(12, 8))
        ax.plot(df['epoch'], df['val/mAP_0.5'], 'r-', linewidth=2, label='mAP@0.5')
        ax.plot(df['epoch'], df['val/mAP_0.5:0.95'], 'b-', linewidth=2, label='mAP@0.5:0.95')
        
        ax.set_title(f'Validation mAP - {self.experiment_name}', fontsize=14)
        ax.set_xlabel('Epoch', fontsize=12)
    ...
    def _create_lr_plot(self, df: pd.DataFrame):
        ...
        fig, ax = plt.subplots(figsize=(12, 8))
        ax.plot(df['epoch'], df['lr/pg0'], 'g-', linewidth=2)
        
        ax.set_title(f'Learning Rate Schedule - {self.experiment_name}', fontsize=14)
        ax.set_xlabel('Epoch', fontsize=12)
    ...
    def log_model_info(self, model: torch.nn.Module, device: torch.device):
        ...
        with open(self.logs_dir / "model_info.txt", 'w', encoding='utf-8') as f:
...
class EnhancedLogger:
    ...
    def __init__(self, save_dir: str, experiment_name: str = "rtdetr_experiment", 
                 num_classes: int = 80, log_interval: int = 50, max_step_history: int = 1000):
        self.logger = YOLOStyleLogger(save_dir, experiment_name, num_classes, log_interval, max_step_history)
    ...
    def log_train_step(self, epoch: int, step: int, metrics: Dict[str, float], 
        ...
        self.logger.log_train_step(epoch, step, metrics, lr, batch_size, time_per_step)
    
    def log_train_epoch(self, epoch: int, metrics: Dict[str, float], 
                       total_time: float, samples_per_sec: float):
        self.logger.log_train_epoch(epoch, metrics, total_time, samples_per_sec)
    
    def log_val_epoch(self, epoch: int, metrics: Dict[str, float], 
                     total_time: float, samples_per_sec: float):
        self.logger.log_val_epoch(epoch, metrics, total_time, samples_per_sec)
    
    def save_training_summary(self, model_info: Dict[str, Any], 
                            best_metrics: Dict[str, Any],
                            total_training_time: float):
        self.logger.save_training_summary(model_info, best_metrics, total_training_time)
    
    def create_training_plots(self):
        self.logger.create_training_plots()
    
    def log_model_info(self, model: torch.nn.Module, device: torch.device):
        return self.logger.log_model_info(model, device)
